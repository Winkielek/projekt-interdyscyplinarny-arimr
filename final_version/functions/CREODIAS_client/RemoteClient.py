...
class RemoteClient:
    ...
    def __init__(self, host: str, username: str, ssh_key_filepath: str, remote_path: str = ''):
        self.host = host
    ...
    def download_file(self, remote_path: str, local_path: str = '', allow_overwrite: bool = False):
        """
        ...
        if not allow_overwrite:
            ...
            if os.path.exists(filepath):
                if self._check_local_and_remote_file(remote_path, local_path, filename):
                    print(f'File {remote_path} exist in {local_path} and hash keys and size are equal. File omitted.')
                    return
                else:
    ...
    def download_dir(self, remote_path: str, local_path: str = '', split_task: bool = True,
        ...
        if split_task:
            ...
            for resp in response:
                ...
                if not os.path.exists(tmp_local_path):
                    ...
                    if not allow_overwrite:
                        ...
                        for f in files:
                            ...
                            if os.path.exists(filepath):
                                if self._check_local_and_remote_file(f, os.path.join(tmp_local_path, directory),
                                                                     filename):
                                    files_locally.append(f)
                        if len(files_locally) == 0:
                            pass
                        elif len(files) == len(files_locally):
                            print(f'Data the same. Remote directory omitted: {tmp_remote_path}.')
                            continue
                        else:
                            print(f'Some data needs to be overwritten in {tmp_local_path}')
                            resp = input_validation(
                ...
                self.scp_client.get(tmp_remote_path, local_path=tmp_local_path, recursive=True)
        else:
    ...
    def bulk_upload(self, files: list, relative_path: str = ''):
        ...
        if len(relative_path) == 0:
            ...
            relative_path = '/' + self.remote_path.strip('/') + '/' + relative_path.strip('/')
        uploads = [self._upload_single_file(file, relative_path) for file in files]
        print(f'Finished uploading {len(uploads)} files to {relative_path} on {self.host}')
    ...
    def _check_size(self, local_path: str, remote_path: str, filename: str = None) -> bool:
        """
    ...
    def _check_local_and_remote_file(self, remote_path: str, local_path: str, filename: str) -> bool:
