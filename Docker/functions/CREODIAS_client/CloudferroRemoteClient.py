...
class CloudferroRemoteClient:
    ...
    def __init__(self, host: str, ssh_key_filepath: str, script_dir: str = 'python_scripts', ignore_check=False):
        ...
        self.remote_client = RemoteClient(host=host,
                                          username='eouser',
    ...
    def find_prepare_and_download(self, arguments: dict):
        ...
        # print(query)
        if self.is_active():
            resp = input('Other preparation script is running. Do you want to continue with your query?[y/n] ')
    ...
    def remove_data(self):
        ...
        :return: None
        """
        response = self.remote_client.execute_command('ls {}'.format(self.script_dir + '/out/'))
        prepared_idx = []
        for itr, item in enumerate(response):
            resp = self.remote_client.execute_command('ls {} | grep done'.format(self.script_dir + '/out/' + item))
            if len(resp) == 1:
                resp = self.remote_client.execute_command('ls {} -A'.format(self.script_dir + '/out/' + item))
        ...
        if len(prepared_idx) > 0:
            ...
            print('There is no fully prepared dataset. Start data preparation using -f argument '
    ...
    def download_prepared(self):
        ...
        if len(prepared_idx) > 0:
            ...
            path = self.script_dir + '/out/' + response[selected]
            self.remote_client.download_dir(remote_path=path, local_path=download_folder)
        else:
            print('There is no fully prepared dataset. Start data preparation using -f argument '
                  'or wait if there is ongoing process')

    def download_during(self):
        response = self.remote_client.execute_command('ls {} -t | head -n1'.format(self.script_dir + '/out/'))
        out_dir = response[0]
        ...
        while self.is_active():
            time.sleep(10)
            print('10s ping')
            response = self.remote_client.execute_command('ls {} -Alt'.format(self.script_dir + '/out/' + out_dir))
            if len(response) > 2:
                ...
                for resp in response:
                    if str(resp).startswith('d'):
                        ...
                        if resp.find('SAFE') > 0 and last_modified > dir_modified + datetime.timedelta(0, 60):
                            record_name = tmp[-1]
        ...
        response = self.remote_client.execute_command('ls {} -Al'.format(self.script_dir + '/out/' + out_dir))
        for resp in response:
            if str(resp).startswith('d'):
        ...
        if not self.is_active():
            ...
            for resp in response:
                if resp.find('Detached') >= 0:
                    ...
                    tmp = self.remote_client.execute_command('screen -S {} -X quit'.format(pid))
                    time.sleep(0.1)
        ...
        if len(version_files) == 1:
            ...
            if len(response) == 1:
                ...
                if response[0] == version:
                    if len(self.python_files) <= len(python_files) and len(self.shell_files) <= len(shell_files):
                        if self._files_list_check(self.python_files, python_files) and self._files_list_check(
